...
def fix_file():
    ...
    for i, line in enumerate(lines):
        if 'def connect_social_account(' in line:
            start_idx = i
            break
    ...
    for i in range(start_idx, len(lines)):
        if 'def social_oauth_callback(' in line:
            end_idx = i - 3
            break
    ...
    correct_content = '''def connect_social_account(
...
'''
    new_lines = lines[:start_idx] + [correct_content + '\n'] + lines[end_idx+1:]
