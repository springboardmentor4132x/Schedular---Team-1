...
async def publish_with_provider(db: Session, post: Post) -> list[PublishingLog]:
    ...
    for platform in platforms:
        ...
        try:
            ...
            # Facebook publishes through the Facebook Page access token,
            # not the user's normal Facebook access token.
            if platform == "facebook":
                ...
                if not account.page_id:
                    log = PublishingLog(
                        ...
                        platform=platform,
                        status="failed",
                        error_message="Facebook Page ID is missing.",
                    )
                    db.add(log)
                    logs.append(log)
                    continue

                if not account.page_access_token_encrypted:
                    log = PublishingLog(
                        post_id=post.id,
                        platform=platform,
                        status="failed",
                ...
                page_access_token = decrypt_token(
                ...
                if not page_access_token:
                    log = PublishingLog(
                        ...
                        platform=platform,
                        status="failed",
                        error_message="Facebook Page access token could not be decrypted.",
                    )
                    db.add(log)
                    logs.append(log)
                    continue
...
def claim_due_post(db: Session, post_id: int) -> Post | None:
    ...
    if post is None or post.scheduled_for is None:
        return None

    now = datetime.now(timezone.utc)
    scheduled_for = to_utc(post.scheduled_for)
