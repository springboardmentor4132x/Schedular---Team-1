...
async def ensure_valid_token(db: Session, account: SocialAccount) -> str:
    ...
    if not access_token:
        raise HTTPException(
            status_code=401,
            detail=(
                f"{account.platform} access token is missing. "
                "User must reconnect."
            ),
        )
    ...
    # If expiry information is not available,
    # use the existing access token.
    if account.token_expires_at is None:
        logger.info(
            "SocialAccount id=%s has no token_expires_at; "
    ...
    if token_expires_at.tzinfo is None:
        ...
        token_expires_at = token_expires_at.astimezone(
            timezone.utc
        )
    ...
    logger.info(
        "SocialAccount id=%s token expired=%s "
        "(needs_refresh=%s)",
        account.id,
        is_expired,
        needs_refresh,
    ...
    # Token is valid for more than 10 minutes.
    if not needs_refresh:
        return access_token

    logger.info(
        "SocialAccount id=%s token is expired or "
        "expiring within 10 minutes; attempting refresh",
        account.id,
    )

    # Token expired or is expiring soon.
    ...
    if not refresh_token:
        raise HTTPException(
            status_code=401,
            detail=(
                f"{account.platform} token expired and "
    ...
    if account.platform == "linkedin":
        new_token_data = await _refresh_linkedin_token(
            refresh_token
        )
    else:
        raise HTTPException(
            status_code=500,
            detail=(
                f"Token refresh not yet implemented "
                f"for {account.platform}"
            ),
        )
