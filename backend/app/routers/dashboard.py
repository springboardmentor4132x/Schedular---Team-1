...
def _oauth_configuration(platform: str) -> tuple[str, str]:
    ...
    if not client_id or not client_secret:
        raise HTTPException(
            status_code=400,
            detail=f"{platform.title()} OAuth credentials are not configured.",
        )
...
def _identity_fields(platform: str, identity: dict) -> tuple[str | None, str | None]:
    ...
    if platform in {"facebook", "instagram"}:
        return identity.get("name") or identity.get("username") or identity.get(
            "id"
        ) or identity.get("user_id"), identity.get("email")
    if platform == "pinterest":
        return (
            identity.get("username")
            or identity.get("profile_image")
            or identity.get("account_type"),
            None,
        )
    return identity.get("name") or identity.get("localizedFirstName") or identity.get(
        "email"
    ), identity.get("email") or identity.get("emailAddress")
...
def _exchange_authorization_code(
    ...
    if platform == "facebook":
        data.update({"client_id": client_id, "client_secret": client_secret})
        return _read_oauth_json(
            f"https://graph.facebook.com/v24.0/oauth/access_token?{urlencode(data)}"
        )
...
def _fetch_provider_identity(platform: str, access_token: str) -> dict:
    ...
    if platform == "facebook":
        return _read_oauth_json(
            "https://graph.facebook.com/v24.0/me?fields=id,name,email", headers=headers
        )
    if platform == "instagram":
        return _read_oauth_json(
            "https://graph.instagram.com/me?fields=user_id,username,account_type",
            headers=headers,
        )
    return _read_oauth_json(
...
def _activity(
    db: Session, user_id: int, activity: str, platform: str | None = None
) -> None:
...
def _account_payload(account: SocialAccount | None, platform: str) -> dict:
    permissions = [
        item for item in (account.permissions if account else "").split(",") if item
    ]
    return {
        "platform": platform,
...
def get_dashboard(
    ...
    return {
        "syncStatus": {
            ...
            "nextSync": None,
            "totalConnected": sum(
                account.status == "connected" for account in accounts
            ),
            "apiHealth": "healthy",
        },
        "scheduledPosts": [
            {
                ...
                "platforms": loads(post.platforms),
...
def get_dashboard_summary(
    ...
    return {
        ...
        "connectedPlatforms": db.query(SocialAccount)
...
def update_profile(
    ...
    duplicate = (
        db.query(User).filter(User.email == payload.email, User.id != user.id).first()
    )
    if duplicate:
    ...
    user.full_name, user.email, user.phone = (
        f"{payload.firstName.strip()} {payload.lastName.strip()}".strip(),
        str(payload.email),
    ...
    profile.timezone, profile.bio, profile.language = (
...
async def upload_avatar(
    ...
    return {
        "success": True,
        "avatarUrl": f"data:{avatar.content_type};base64,{base64.b64encode(content).decode()}",
    }
...
def update_settings(
    ...
    if payload.section == "general":
        ...
        for source, target in (
            ...
            ("publishingAlerts", "publishing_alerts"),
            ("campaignAlerts", "campaign_alerts"),
...
def social_oauth_callback(
    ...
    if error or not code or not state:
        return RedirectResponse(
            f"{settings.FRONTEND_URL.rstrip('/')}/connect-apps?oauth_error={platform}"
        )
    try:
    ...
    if not access_token:
        raise HTTPException(
            status_code=502, detail="Social provider did not return an access token."
        )
    identity = _fetch_provider_identity(platform, access_token)
    _save_oauth_account(db, user.id, platform, token_data, identity)
    return RedirectResponse(
        f"{settings.FRONTEND_URL.rstrip('/')}/connect-apps?connected={platform}"
    )
...
def refresh_social_account(
    ...
    if not refresh_token or platform not in {"youtube", "x", "pinterest"}:
        return {
            "success": False,
            "message": f"{platform.title()} requires reconnecting when its access token expires.",
        }
    client_id, client_secret = _oauth_configuration(platform)
    headers = (
        {"Authorization": f"Basic {_basic_auth(client_id, client_secret)}"}
        if platform in {"x", "pinterest"}
        else None
    )
...
def delete_account(
    ...
    post_ids = [
        post_id
        for (post_id,) in db.query(Post.id).filter(Post.owner_id == user.id).all()
    ]
    campaign_ids = [
        campaign_id
    ...
    if post_ids:
        db.query(AnalyticsMetric).filter(AnalyticsMetric.post_id.in_(post_ids)).delete(
            synchronize_session=False
        )
        db.query(PublishingLog).filter(PublishingLog.post_id.in_(post_ids)).delete(
            synchronize_session=False
        )
        db.query(CampaignPost).filter(CampaignPost.post_id.in_(post_ids)).delete(
            synchronize_session=False
        )
    if campaign_ids:
        db.query(CampaignPost).filter(
    ...
    db.query(Campaign).filter(Campaign.owner_id == user.id).delete(
        synchronize_session=False
    )
    db.query(MediaAsset).filter(MediaAsset.owner_id == user.id).delete(
        synchronize_session=False
    )
    db.query(Report).filter(Report.owner_id == user.id).delete(
        synchronize_session=False
    )
    db.query(CollaborationRequest).filter(
        (CollaborationRequest.business_user_id == user.id)
        | (CollaborationRequest.requested_by_user_id == user.id)
    ).delete(synchronize_session=False)
