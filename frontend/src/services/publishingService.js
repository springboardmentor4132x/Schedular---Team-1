...
const publishingService = {
  ...
  getPublishingLogs: async (skip = 0, limit = 100) => {
    const { data } = await api.get(`/publishing/logs?skip=${skip}&limit=${limit}`);
    return Array.isArray(data.data?.items) ? data.data.items : (data.data || []);
  },

  /** Get all posts in the publishing queue (status = scheduled or queued). */
  getQueue: async () => {
    const { data } = await api.get('/publishing/queue');
    return Array.isArray(data.data) ? data.data : (data?.items ?? []);
  },

  /** Get failed posts (status = failed). */
  getFailedPosts: async () => {
    const { data } = await api.get(`/publishing/logs?skip=0&limit=200`);
    const items = Array.isArray(data.data?.items) ? data.data.items : (data.data || []);
    return items.filter((p) => p.status === 'failed');
  },

  /** Retry a failed post (re-schedule it). */
  retryPost: async (postId) => {
    const { data } = await api.post(`/publishing/retry/${postId}`);
    return data;
  },

  /** Delete a post. */
  deletePost: async (postId) => {
    const { data } = await api.delete(`/publishing/${postId}`);
    return data;
  },

  /** Get social accounts connected to the current user. */
  getSocialAccounts: async () => {
    const { data } = await api.get('/publishing/accounts');
